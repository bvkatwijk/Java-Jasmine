...
/**
 * Test case for validation of test behaviour with nested focus groups
 */
public class MultipleNestedFDescribeTest {
	...
	@Getter private static final AtomicBoolean d_fd_fd_it_didRun = new AtomicBoolean(false);
	@Getter private static final AtomicBoolean d_fd_fd_d_it_didRun = new AtomicBoolean(false);
	...
	private static final JasmineInternalTest TEST = new JasmineInternalTest(){{
		describe("describe", () -> {
			...
			fdescribe("fdescribe", () -> {
				...
				fdescribe("fdescribe", () -> {
					it("it", () -> {
						d_fd_fd_it_didRun.set(true);
					});
					describe("describe", () -> {
						it("it", () -> {
							d_fd_fd_d_it_didRun.set(true);
						});
					});
				});
			});
	...
	@Test
	public void d_fd_it_didRun_shouldBeFalse() {
		Assert.assertFalse(d_fd_it_didRun.get());
	}

	@Test
	public void d_fd_d_it_didRun_shouldBeFalse() {
		Assert.assertFalse(d_fd_d_it_didRun.get());
	}

	@Test
	public void d_fd_fd_it_didRun_shouldBeTrue() {
		Assert.assertTrue(d_fd_fd_it_didRun.get());
	}

	@Test
	public void d_fd_fd_d_it_didRun_shouldBeTrue() {
		Assert.assertTrue(d_fd_fd_d_it_didRun.get());
	}
